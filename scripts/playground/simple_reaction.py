...
def main():
    ...
    df = pd.DataFrame(data=sim_result[1], columns=[str(s.name) for s in model.species])
    df['time'] = np.arange(config['simulation']['num_steps'] * config['simulation']['delta_t'], step=
